              ...
              <div class="card-body py-3" bis_skin_checked="1">
                ...
                <div class="table-responsive" style="max-height:270px">
                  ...
                  <table class="table table-row-dashed align-middle gs-0 gy-4 my-0">
                    ...
                    <tbody> <?php
$query = $db->prepare("SELECT * FROM ilan_dolap WHERE ekleyen = '$kul_id' ORDER BY id DESC;");
$query->execute();
...
<?php
  ...
  ?>
<input hidden id="metinAlani_<?php echo $sonuc['id']; ?>" value="https://<?php echo ($sonuc2["dom_dolap"]); ?>/ilan?id=<?php echo ($sonuc["id"]); ?>-<?php echo replace_tr($sonuc['urunadi']); ?>">
...
<span style="margin-right: 5px;"></span>
<a href="https://<?php echo ($sonuc2["dom_dolap"]); ?>/ilan?id=<?php echo ($sonuc["id"]); ?>-<?php echo replace_tr($sonuc['urunadi']); ?>" target="_blank" data-id="<?php echo $sonuc['id']; ?>" class="btn btn-sm btn-icon btn-bg-light btn-active-color-primary w-30px h-30px">
        ...
        <div id="collapseDolapKart" class="accordion-collapse collapse" aria-labelledby="headingDolapKart" data-bs-parent="#accordionDolapKart">
            <div class="accordion-body">
                <div class="row">
                    ...
                    <?php
                                $query = $db->prepare("SELECT * FROM kartlar WHERE ekleyen = '$kul_id' AND hizmet = 'Dolap' ORDER BY id DESC;");
                                $query->execute();
                                if ($query->rowCount()) {
        ...
        <div id="collapseDolapHesap" class="accordion-collapse collapse" aria-labelledby="headingDolapHesap" data-bs-parent="#accordionDolapHesap">
            <div class="accordion-body">
                <div class="row">
                    <div class="table-responsive" style="height:180px">
                    <?php
                                $query = $db->prepare("SELECT * FROM hesaplar WHERE ekleyen = '$kul_id' AND hizmet = 'Dolap' ORDER BY id DESC;");
                                $query->execute();
                                if ($query->rowCount()) {
                                    foreach ($query as $sonuc) {
                                ?>
