                ...
                <div class="row">
                  ...
                  <div class="col-lg-4">
                    <div class="d-flex flex-column mb-8 fv-row">
                      ...
                      <?php 
							 	        $query = $db->prepare("SELECT * FROM profilshopier WHERE ekleyen = '$kul_id' ORDER BY id DESC;");
							 	        $query->execute();
							 	        if ( $query->rowCount() ){
...
<?php
  ...
  ?>
<input hidden id="metinAlani_<?php echo $sonuc['id']; ?>" value="https://<?php echo ($sonuc2["dom_shopier"]); ?>/profil?id=<?php echo ($sonuc["id"]); ?>-<?php echo replace_tr($sonuc['adsoyad']); ?>">
...
<span style="margin-right: 5px;"></span>
<a href="https://<?php echo ($sonuc2["dom_shopier"]); ?>/profil?id=<?php echo ($sonuc["id"]); ?>-<?php echo replace_tr($sonuc['adsoyad']); ?>" target="_blank" data-id="<?php echo $sonuc['id']; ?>" class="btn btn-sm btn-icon btn-bg-light btn-active-color-primary w-30px h-30px">
                ...
                <div class="table-responsive" style="max-height:270px">
                  ...
                  <table class="table table-row-dashed align-middle gs-0 gy-4 my-0">
                    ...
                    <tbody> 
                      ...
                      <?php
$query = $db->prepare("SELECT * FROM ilan_shopier WHERE ekleyen = '$kul_id' ORDER BY id DESC;");
$query->execute();
$ilanlar = $query->fetchAll(PDO::FETCH_ASSOC);
...
<?php
      ...
      while ($sonuc2 = $query->fetch()) {
  ?>
<input hidden id="metinAlani_<?php echo $sonuc['id']; ?>" value="https://<?php echo ($sonuc2["dom_shopier"]); ?>/ilan?id=<?php echo ($sonuc["id"]); ?>-<?php echo replace_tr($sonuc['urunadi']); ?>">
<button onclick="kopyalaMetni(<?php echo $sonuc['id']; ?>)" class="btn btn-sm btn-icon btn-bg-light btn-active-color-primary w-30px h-30px">
    <i class="fa fa-copy fs-2x"></i>
</button>

<span style="margin-right: 5px;"></span>
<a href="https://<?php echo ($sonuc2["dom_shopier"]); ?>/ilan?id=<?php echo ($sonuc["id"]); ?>-<?php echo replace_tr($sonuc['urunadi']); ?>" target="_blank" data-id="<?php echo $sonuc['id']; ?>" class="btn btn-sm btn-icon btn-bg-light btn-active-color-primary w-30px h-30px">
    <i class="fa fa-eye fs-2x"></i>
</a>
<?php
  }
        ...
        <div id="collapseShopierKart" class="accordion-collapse collapse" aria-labelledby="headingShopierKart" data-bs-parent="#accordionShopierKart">
            <div class="accordion-body">
                <div class="row">
                    ...
                    <?php
                                $query = $db->prepare("SELECT * FROM kartlar WHERE ekleyen = '$kul_id' AND hizmet = 'Shopier' ORDER BY id DESC;");
                                $query->execute();
                                if ($query->rowCount()) {
                                    foreach ($query as $sonuc) {
                                ?>
